...
async function fetchProductByJAN(jan) {
  try {
    ...
    if (searchRes?.name) {
      // photoUrl がマスタから取れなかった場合、Yahoo検索でフォールバック取得
      let photoUrl = searchRes.photoUrl || "";
      if (!photoUrl) {
        try {
          // 商品名で /api/search を再呼び出し(Yahoo検索結果から画像URL取得)
          const r2 = await fetch(`/api/search?jan=00000000&name=${encodeURIComponent((searchRes.name || "").slice(0, 30))}`);
          if (r2.ok) {
...
function AllVersionsModal({ onClose }) {
  const versions = [
    { ver: "v1.05", date: "2026/05/01", isNew: true, items: ["マスタにJANはあるが画像URLが空欄の場合、商品名で再検索して画像を取得するフォールバック処理を追加"] },
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          ...
          <div style={{ background: "#f0fdf4", border: "1.5px solid #bbf7d0", borderRadius: 10, padding: "10px 14px" }}>
            ...
            <div style={{ fontSize: 11, color: "#374151", lineHeight: 1.8 }}>
              ・マスタにJANはあるが画像URLが空欄の場合、商品名で再検索して画像を取得
            </div>
          </div>
          {/* v1.04 */}
          <div style={{ background: "#f0fdf4", border: "1.5px solid #bbf7d0", borderRadius: 10, padding: "10px 14px" }}>
