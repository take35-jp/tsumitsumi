...
- **v1.42**: モデラーズアルバムの一覧ヘッダーに「**このアプリを共有**」ボタンを追加（バックアップ(DL)アイコンの左、HELPの並び）。`ModelerAlbum` 内に `maShareApp()` を新設し、share-nodes 風 SVG アイコンの `ma.ghost` ボタンから起動。共有先 URL は `https://tsumitsumi.vercel.app/?modeler`（専用アイコン/名称で直接アルバムが開く導線）、テキストは `作品ポートフォリオを端末内だけで作れる「モデラーズアルバム」\n#モデラーズアルバム #TSUMITSUMI`。`navigator.share` 対応端末（主にスマホ）はネイティブ共有シート、非対応（PC等）は `navigator.clipboard.writeText` で URL コピー＋alert、clipboard も不可なら X intent にフォールバック。実機(headless)で配置順 `[共有][バックアップ][HELP][ロゴ]`・コピー動作・ビルド通過を確認済み。
