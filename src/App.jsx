...
${idLine}#積みプラ #プラモデル #ツミツミ #気になるツミはありますか`;
  ...
  const handleGenerateImages = async () => {
    ...
    try {
      ...
      // ダウンロード完了後にXを開く
      const tweetText = buildTweetForImage(targetKits.length, blobs.length);
      setTimeout(() => {
    ...
    <div style={xs.wrap}>
      ...
      {pending.length === 0 ? <div style={xs.empty}>積みプラが登録されていません</div> : (<>
        ...
        <div style={{ background: "#f0fdf4", border: "1.5px solid #bbf7d0", borderRadius: 10, padding: "14px", marginBottom: 12 }}>
          ...
          {generatedCount > 0 && (
            <div style={{ marginTop: 10, background: "#dcfce7", borderRadius: 8, padding: "10px 12px" }}>
              ...
              <div style={{ fontSize: 11, color: "#166534", lineHeight: 1.6 }}>
                Xが開いたら「画像を追加」から保存した画像を選んで投稿してください。
              </div>
              <button style={{ width: "100%", marginTop: 8, padding: "10px 0", background: "#000", color: "#fff", border: "none", borderRadius: 8, fontSize: 13, fontWeight: 700, cursor: "pointer" }}
                onClick={() => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(buildTweetForImage(targetKits.length, generatedCount))}`, "_blank")}>
                𝕏 Xを開いて投稿する
              </button>
            </div>
          )}
