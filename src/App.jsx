...
const MIX_LS_KEY = "tsumitsumi_mix_recipes"; // 名前付き調色レシピ集（作品を跨いで再利用）
...
function ModelerAlbum({ onClose, tagMasterList, setTagMasterList, kits, setKits, paintPreview, onOpenPaints }) {
  ...
  useEffect(() => {
    ...
    try { const m = JSON.parse(localStorage.getItem(MIX_LS_KEY) || "[]"); if (Array.isArray(m)) setMixRecipes(m); } catch (e) {}
    try { if (!localStorage.getItem(MA_INTRO_KEY)) setMaIntro(true); } catch (e) {} // 初回アクセスのみ案内ポップアップ
  }, []);
  useEffect(() => {
    if (!recipesLoaded.current) { recipesLoaded.current = true; return; } // 初回マウントの保存はスキップ
    try { localStorage.setItem(MIX_LS_KEY, JSON.stringify(mixRecipes)); } catch (e) {}
  }, [mixRecipes]);
  ...
  // ---- 名前付き調色レシピ集（Phase D）----
  const saveMixToLibrary = (mix) => {
    if (!mix || !(mix.parts || []).length) { alert("材料が空です。先に材料を追加してください。"); return; }
    const name = (window.prompt("レシピ名を付けて保存します", mix.label || "") || "").trim();
    if (!name) return;
    const rec = { id: makePhotoId(), name, resultSwatch: mix.resultSwatch || "#9aa0a6", note: mix.note || "", parts: (mix.parts || []).map(p => ({ paintId: p.paintId || null, name: p.name || "", swatch: p.swatch || "#999", ratio: p.ratio || "" })), savedAt: Date.now() };
    setMixRecipes(prev => [rec, ...prev]);
    alert(`「${name}」をレシピ集に保存しました。`);
  };
  const recallRecipe = (rec) => {
    setDraft(d => ({ ...d, mixes: [...(d.mixes || []), { id: makePhotoId(), label: rec.name || "", resultSwatch: rec.resultSwatch || "#9aa0a6", note: rec.note || "", parts: (rec.parts || []).map(p => ({ ...p })) }] }));
    setRecallOpen(false);
  };
  const deleteRecipe = (id) => { if (!window.confirm("このレシピをレシピ集から削除しますか？")) return; setMixRecipes(prev => prev.filter(r => r.id !== id)); };
  ...
  // ---- 調色レシピ集から呼び出す ----
  const renderRecallRecipes = () => {
    if (!recallOpen) return null;
    return (
      <div style={{ position: "fixed", inset: 0, zIndex: 460, background: "#fff", display: "flex", flexDirection: "column", fontFamily: MA_FONT }}>
        <div style={ma.bar}>
          <button style={ma.ghost} onClick={() => setRecallOpen(false)}>閉じる</button>
          <div style={{ fontSize: 13, fontWeight: 800, letterSpacing: "0.1em" }}>調色レシピ集</div>
          <div style={{ width: 64 }} />
        </div>
        <div style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch", padding: "12px 18px 40px", maxWidth: 940, margin: "0 auto", width: "100%" }}>
          {mixRecipes.length === 0 && (
            <div style={{ color: "#999", fontSize: 13, lineHeight: 1.9, textAlign: "center", padding: "40px 0" }}>
              保存された調色レシピがありません。<br />調色レシピの「★保存」で登録できます。
            </div>
          )}
          {mixRecipes.map(r => (
            <div key={r.id} style={{ border: "1px solid #111", padding: 12, marginBottom: 10 }}>
              ...
              <div style={{ fontSize: 13, color: "#333", lineHeight: 1.8 }}>
                {(r.parts || []).map((p, i) => <span key={i}>{i > 0 ? " ＋ " : ""}{p.name || "?"}{p.ratio ? ` ×${p.ratio}` : ""}</span>)}
              </div>
              {r.note && <div style={{ fontSize: 11, color: "#888", marginTop: 6 }}>{r.note}</div>}
            </div>
          ))}
        </div>
      </div>
        ...
        <div style={ma.body}>
          ...
          <div style={{ marginBottom: 26 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              ...
              <button style={ma.ghost} onClick={() => setRecallOpen(true)}>レシピ集から呼び出す（{mixRecipes.length}）</button>
            </div>
          </div>
          </>)}
...
// My PALETTE ロゴ。public/MyPALETTE2.（png/jpg/jpeg/webp）があればそれを使用。無ければ黒地のフォールバックSVG。
const MYPALETTE_LOGO_SRCS = ["/MyPALETTE2.png", "/MyPALETTE2.jpg", "/MyPALETTE2.jpeg", "/MyPALETTE2.webp"];
function MyPaletteLogo({ size = 32 }) {
  const [idx, setIdx] = useState(0);
  if (idx < MYPALETTE_LOGO_SRCS.length) {
    return <img src={MYPALETTE_LOGO_SRCS[idx]} alt="My PALETTE" onError={() => setIdx(idx + 1)} style={{ width: size, height: size, objectFit: "contain", display: "block", flexShrink: 0 }} />;
  }
  return (
...
function PaintStock({ onClose, onOpenModeler }) {
  ...
  const [pHelp, setPHelp] = useState(false); // 取扱説明書
    ...
    <div style={{ position: "fixed", inset: 0, zIndex: 420, background: "#fff", display: "flex", flexDirection: "column" }}>
        ...
        {catList.map(({ c, isTop, brand, swatch }, i) => {
          ...
          return (
            <div key={i} onClick={() => catMulti ? toggleCatSel(c, isTop) : pickCatalog(c, isTop)} style={{ ...ps.row, marginBottom: 6, border: sel ? "2px solid #111" : ps.row.border, background: sel ? "#f3f4f6" : "#fff" }}>
              ...
              <div style={{ fontSize: 16, fontWeight: 800, color: sel ? "#111" : "#9ca3af", flexShrink: 0 }}>{catMulti ? (sel ? "✓" : "＋") : "＋"}</div>
            </div>
          );
        })}
      ...
      {catMulti && (
        <div style={{ borderTop: "1px solid #e5e7eb", padding: "10px 14px", background: "#fff" }}>
          <button style={{ ...ps.black, width: "100%", opacity: Object.keys(catSel).length ? 1 : 0.5 }} disabled={!Object.keys(catSel).length} onClick={addCatalogSelected}>
            選択中 {Object.keys(catSel).length} 件を追加
          </button>
        </div>
      )}
    </div>
  ...
  if (editing) {
    ...
    return (
      <div style={ps.wrap}>
        ...
        <div style={ps.body}>
          <button style={{ ...ps.black, width: "100%", marginBottom: 12 }} onClick={() => openCatalog(e, false)}>PAINT GUIDE・TOPCOAT GUIDEから自動入力</button>
  ...
  return (
    <div style={ps.wrap}>
      {/* タイトル（My PALETTE ロゴ） */}
      <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "10px 14px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      ...
      <div style={{ background: "#fff", borderBottom: "1px solid #e5e7eb", padding: "8px 14px", display: "flex", flexDirection: "column", gap: 6 }}>
        ...
        <button style={{ ...ps.black, width: "100%" }} onClick={() => openCatalog(null, true)}>PAINT GUIDE・TOPCOAT GUIDEから探して追加（複数可）</button>
      </div>
      ...
      <div style={{ position: "fixed", right: 18, bottom: 22, display: "flex", flexDirection: "column", alignItems: "stretch", gap: 8, zIndex: 2 }}>
        {(() => {
          ...
          return (<>
            <button style={{ ...fab, background: "#fff", color: "#111", border: "1.5px solid #111" }} onClick={() => { scanTargetRef.current = "paint"; setScanning(true); }}>スキャン登録</button>
            <button style={fab} onClick={() => setEditing(blank())}>直接入力</button>
            <button style={fab} onClick={() => setEditing(blankMix())}>調色レシピ</button>
          </>);
        })()}
      </div>
      ...
      {pHelp && (
        <div style={{ position: "fixed", inset: 0, zIndex: 440, background: "#fff", display: "flex", flexDirection: "column" }}>
          <div style={ps.bar}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}><MyPaletteLogo size={24} /><span style={{ fontSize: 14, fontWeight: 800 }}>My PALETTE の使い方</span></div>
            <button style={ps.ghost} onClick={() => setPHelp(false)}>閉じる</button>
          </div>
          <div style={{ flex: 1, overflowY: "auto", WebkitOverflowScrolling: "touch", padding: "16px 16px 50px", maxWidth: 720, margin: "0 auto", width: "100%", fontSize: 13, lineHeight: 1.85, color: "#333" }}>
            {[
              ["My PALETTE とは", "持っている塗料・トップコートを端末内で管理する「自分のパレット」です。データはこのブラウザにのみ保存され、サーバーには送信されません。"],
              ["塗料を登録する（3つの方法）", "下部のボタンから登録します。①スキャン登録＝バーコードを読み取り、商品名を自動取得。②直接入力＝メーカー・色名などを手入力。③調色レシピ＝自分で混ぜた色を記録（後述）。さらに「PAINT GUIDE・TOPCOAT GUIDEから探して追加」で大全データから検索し、複数まとめて登録できます。"],
              ["色（系統）と並び替え", "色は「白／赤系／青系…」など系統で選びます（調色レシピのみカラーピッカーで自由指定）。一覧は色フィルタ＋並び替え（登録順／色順／メーカー順／名前順）で整理できます。"],
              ["Amazonで補充", "保存時にAmazonの商品リンク・画像を自動取得します。一覧右の「補充」ボタンから購入ページを開けます。"],
              ["調色レシピ", "下部「調色レシピ」から、自分で混ぜた色を登録できます。材料の塗料をスキャンまたは手入力で追加し、それぞれの割合（％）を記録。仕上がりの色はカラーピッカーで自由に指定できます。"],
              ["モデラーズアルバムとの連携", "モデラーズアルバムの各作品に「使用した塗料」「調色レシピ」を記録できます。色は My PALETTE に登録した塗料から選ぶか、直接入力できます。"],
              ["バックアップ", "右上のバックアップ（↓）アイコンから、データの書き出し（JSON）と読み込み（復元・統合）ができます。機種変更・初期化の前に書き出しておきましょう。別のブラウザへ移行する場合も書き出し→読み込みで移せます。"],
              ["データについての注意", "データは端末（ブラウザ）内のみに保存されます。ブラウザのデータ削除や別ブラウザではデータは共有されません。大切なデータは定期的にバックアップしてください。"],
            ].map(([h, b], i) => (
              <div key={i} style={{ marginBottom: 18 }}>
                <div style={{ fontWeight: 800, color: "#111", fontSize: 14, borderLeft: "4px solid #111", paddingLeft: 8, marginBottom: 6 }}>{h}</div>
                <div>{b}</div>
              </div>
            ))}
          </div>
