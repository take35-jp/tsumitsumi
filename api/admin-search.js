                                                                                                      ...
                                                                                                      return true;
                                                                                                              ...
                                                                                                              return res.json({ items, total: data?.totalResultsAvailable || 0 });
                                                                                                                    ...
                                                                                                                    return res.status(500).json({ error: String(e) });
                                                                                                                      ...
                                                                                                                      async function fixImages(req, res) { if (req.query.debug === "env") return res.json({ url: !!SUPABASE_URL, urlLen: SUPABASE_URL?.length, key: !!SUPABASE_SERVICE_ROLE_KEY, keyLen: SUPABASE_SERVICE_ROLE_KEY?.length, async function fixImages(req, res) { if (req.query.debug === "env") return res.json({ url: !!SUPABASE_URL, urlLen: SUPABASE_URL?.length, key: !!SUPABASE_SERVICE_ROLE_KEY, keyLen: SUPABASE_SERVICE_ROLE_KEY?.length, yahoo: !!YAHOOyahoo: !!YAHOO_CLIENT_ID }); if (req.query.debug === "sel") { const hh = { apikey: SUPABASE_SERVICE_ROLE_KEY, Authorization: `Bearer ${SUPABASE_SERVICE_ROLE_KEY}` }; const ss = `${SUPABASE_URL}/rest/v1/products?select=id,jan&image_url=is.null&jan=not.like.PB-*&limit=5&order=id.asc`; const rr = await fetch(ss, { headers: hh }); const tt = await rr.text(); return res.json({ sta: rr.status, txt: tt.slice(0,400), selUrl: ss.slice(0,150) }); }_CLIENT_ID });
