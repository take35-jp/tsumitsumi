...
function guessSeriesAndScale(name) {
  ...
  else if (/\bHGUC\b/i.test(name)) scale = "HG";
  else if (/\bHGCE\b/i.test(name)) scale = "HG";
  ...
  else if (/1\/48/i.test(name))    scale = "1/48";
  else if (/1\/35/i.test(name))    scale = "1/35";
  ...
  else if (/ウルトラマン/i.test(name))               series = "ウルトラマン（バンダイ）";
  else if (/仮面ライダー/i.test(name))               series = "仮面ライダー（バンダイ）";
  else if (/エヴァ|エヴァンゲリオン|EVA/i.test(name)) series = "新世紀エヴァンゲリオン";
  else if (/マクロス/i.test(name))                  series = "マクロス（バンダイ）";
  else if (/スターウォーズ|STAR WARS/i.test(name))  series = "スターウォーズ（バンダイ）";
...
export default async function handler(req, res) {
  const query   = req.query.query || "バンダイ プラモデル ガンプラ";
